...
try:
    ...
    all_data = {stat_name: instance.get_all_data() for stat_name, instance in stats_instances.items()}
    ...
    for team_name in team_names:
        try:
            ...
            processed_data = {stat_name: instance.process_data(all_data[stat_name], team_name) for stat_name, instance in stats_instances.items()}
            if all(team_name.replace(" ", "") == data['TeamName'].replace(" ", "") for data in processed_data.values()):
                merged_data = {key: value for d in processed_data.values() for key, value in d.items()}
                logger.debug(f"Merged data for {team_name}: {json.dumps(merged_data, indent=2)}")
